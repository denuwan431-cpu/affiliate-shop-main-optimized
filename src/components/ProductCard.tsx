    ...
    <div className="bg-white rounded-[45px] border border-slate-100 hover:border-orange-500/20 hover:shadow-[0_40px_80px_-20px_rgba(0,0,0,0.08)] transition-all duration-700 flex flex-col h-full relative group overflow-hidden">
      ...
      <Link href={`/product/${product.id}`} className="block relative aspect-square bg-[#fcfcfc] m-3 rounded-[38px] overflow-hidden">
        <Image 
          ...
          alt={displayName} 
          fill 
          className="object-contain p-10 group-hover:scale-110 transition-transform duration-1000 ease-out" 
        />
      </Link>
      ...
      <div className="p-8 pt-4 flex flex-col flex-1">
        ...
        <Link href={`/product/${product.id}`}>
          <h3 className="text-[18px] font-[1000] text-slate-800 line-clamp-2 h-14 leading-[1.2] tracking-tight group-hover:text-orange-600 transition-colors uppercase">
            {displayName}
          </h3>
        </Link>
        ...
        <div className="mt-auto pt-8 space-y-6">
          <div className="flex flex-col">
            <div className="flex items-baseline gap-2">
                <span className="text-3xl font-[1000] text-slate-900 tracking-tighter">Rs. {price.toLocaleString()}</span>
            </div>
            {originalPrice && originalPrice > price && (
              <span className="text-[12px] text-slate-300 line-through font-bold mt-1 opacity-80 uppercase tracking-tighter">WAS Rs. {originalPrice.toLocaleString()}</span>
            )}
          </div>
          ...
          <a
            ...
            className="w-full bg-slate-900 hover:bg-orange-600 text-white text-[11px] font-[1000] py-5 rounded-[22px] flex items-center justify-center gap-3 transition-all active:scale-95 uppercase tracking-[0.2em] shadow-2xl shadow-slate-200 hover:shadow-orange-900/20"
          >
            CHECK DEAL <ArrowRight size={16} className="group-hover:translate-x-1 transition-transform" />
          </a>
        </div>
      </div>
    </div>
