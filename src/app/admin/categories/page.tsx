        ...
        <div className="divide-y">
          {allCats.map((cat) => (
            <div key={cat.id} className="p-5 flex justify-between items-center hover:bg-gray-50">
              <div>
                <div className="font-bold text-gray-800 text-lg">{cat.name}</div>
          ...
          {allCats.length === 0 && <div className="p-10 text-center text-gray-400">No categories added yet.</div>}
        </div>
      </div>
      
      <div className="p-8 bg-blue-50 rounded-3xl border border-blue-100">
        <h2 className="font-bold mb-4 text-blue-900 text-lg">Add New Category</h2>
        <form action={upsertCategory} className="flex flex-col gap-4">
          <input name="name" placeholder="Category Name (e.g. Mobile Accessories)" className="p-3 bg-white border-none rounded-2xl focus:ring-2 focus:ring-blue-500 shadow-sm" required />
